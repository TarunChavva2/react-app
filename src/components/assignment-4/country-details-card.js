...
class Countrydetailscard extends React.Component {
	...
	render() {
		...
		return (
			<div className = "individual-country-background-color" >
			<Header/>
			<button className="back-button" onClick={this.goBackHome}><IoIosArrowRoundBack/>Back</button> {
			...
			<img src={this.state.countryDetails[0].flag} alt="national-flag" className="country-details-image"/>
			  ...
			  <div>
				 ...
				 {return <button className="individual-country-button" onClick={()=>this.redirectToSelectedBorderCountry(item)}>{item}</button>}
				 )}
			  </div>
			  </div>
			</div>
